...
#       self.sigma = jnp.diagonal(self.sigma); True sigma
    ...
    def initialize_GP(self, X, y):
        ...
        self.test_count = jnp.zeros(a) # 1D array with len=a
        ...
        self.A = jnp.linalg.inv(self.K_t + self.eta**2 * jnp.eye(T)) # A^(-1) in fact
        ...
        self.sigma = self.variance * jnp.eye(self.x_star.shape[0]) - self.k_star.T @ self.A @ self.k_star
        self.t = T
    ...
    def update_obs(self, x, y):
        ...
        self.y_t1 = jnp.array([y]) #[] add 1 extra dim to scalar y:scalar - 1D
        #1D arr (1, )
    ...
    def iterate_vars(self):
        self.y = jnp.append(self.y, self.y_t1)
        self.X_t = jnp.append(self.X_t, self.x_t1, axis=0)
        self.k_star = jnp.append(self.k_star, kernel(self.x_t1, self.x_star, self.c, self.kernels), axis=0)  #self.variance, self.gamma, self.kernels, self.matern_nu

        ## update for A eq (27) in note 2.3
        self.A = self.A + self.phi * jnp.outer(self.u, self.u)
        #print("A shape1",self.A.shape)
        self.A = jnp.vstack((self.A, -self.phi*self.u.T))
        #print("A shape2",self.A.shape)
        right = jnp.append(-self.phi*self.u, self.phi)
        #print("A shape3",right.shape)
        self.A = jnp.column_stack((self.A, right))
    ...
    #@profile
    def max_acq(self):
        test_pt = jnp.argmax(self.ucb())

        # val = self.f - jnp.max(self.f) - 1e-4
        # sig = jnp.diagonal(self.sigma)
        # test_pt = jnp.argmax(val * norm.cdf(val / sig) + sig * norm.pdf(val))
        if self.test_count[test_pt] >= 3:  # test_counts start with 0, to show max 3 repeated stim, use "\geq"
            test_pt = jnp.random.choice(jnp.arange(self.x_star.shape[0]))
            logger.info("Selected stimulus exceeds max tests; Choose randomly ---")
            # arange-generate an array range [0--n-1];jnp.rd.chocie:sample a number from the array
    ...
    #@profile
    def ucb(self):
        tau = self.d * jnp.log(self.t + 1e-16) # eq 35
        # import pdb; pdb.set_trace()
        #GP-UCB(x) = µ(x) + (√ντ)σ(x).
        sig = self.sigma
        if jnp.any(sig < 0):
            sig = jnp.clip(sig, 0, jnp.max(sig))
        # if any element in the matrix sig is less than zero
        # jnp.clip(array, a_min, a_max, out=None)->keep non-neg

        fcn = self.f + jnp.sqrt(self.nu * tau) * jnp.sqrt(jnp.diagonal(sig) + 1e-16)
        #diagonal-sd
        return fcn
    ...
    def stopping(self):
        ...
        sig = jnp.diagonal(self.sigma) + 1e-12
        PI = jnp.max(stats.norm.cdf((val) / (sig)))

        # using expected improvement; updated to correct EI calculation
        EI = jnp.max(val * stats.norm.cdf(val / sig) + sig * stats.norm.pdf(val / sig))
        # combined = (EI + PI)*0.5
...
def update_GP_ext(X_t, x_t1, A, y, y_t1, k_star, kvv, c, kernels):#update_GP_ext(X_t, x_t1, A, x_star, eta, y, y_t1, k_star, variance, gamma, kvv, kernels, matern_nu):
    ...
    phi = jnp.linalg.inv(k_t1 + c.eta**2 - k_t.T.dot(u)) # (1,1)
    ...
    df = jnp.squeeze(phi * kuk * (((y.T).dot(u)).T - y_t1)) #(1,1) * 2D arr *1D (1,)
...
def calc_offline_fit(X, y, c, kernels):
    assert isinstance(X, jnp.ndarray) and isinstance(y, jnp.ndarray), "Please convert X and y in numpy array"
    is_X_object = X.dtype == jnp.object_
    is_y_object = y.dtype == jnp.object_
    ...
    if not is_y_multi:
        ...
        print(neuron_len)
    f = jnp.empty((neuron_len, c.x_star.shape[0])) # the number of neurons
    sigma = jnp.empty((neuron_len, c.x_star.shape[0], c.x_star.shape[0]))
    if not is_X_object:
        if not is_y_multi:
    ...
    for neuron in range(neuron_len):  #y.shape[0]):
        ...
        A = jnp.linalg.inv(K_t + c.eta**2 * jnp.eye(T))     # TO DO - look at pinv 
        f.at[neuron].set(k_star.T @ A @ y_neuron)                    # predicted means
        sigma.at[neuron].set(c.var * jnp.eye(c.x_star.shape[0]) - k_star.T @ A @ k_star)    # covariance matrix
        t = T
...
def calc_online_fit(X, y, c, kernels, init_T, end_T):
    ...
    # f = jnp.empty((neuron_len, c.x_star.shape[0])) # the number of neurons
    # sigma = jnp.empty((neuron_len, c.x_star.shape[0], c.x_star.shape[0]))
    f = jnp.empty((online_len, c.x_star.shape[0]))
    sigma = jnp.empty((online_len, c.x_star.shape[0], c.x_star.shape[0]))
    stopping_crits = jnp.empty((online_len, 2))  # getting both EI and PI i assume 
    peak_lc = jnp.empty((online_len, 5))
    for neuron in range(neuron_len):  # number of neurons
        ...
        initial_y = jnp.array(y_neuron[0:init_T_neuron])  #[0:4]
...
def find_peaks(f_array, x_star):
    return x_star[jnp.argmax(f_array, axis = 1)]
...
def mse_on_offline(f_online, f_offline, x_star, of_peak = True):
    ...
    # mse_on_offline_ls = jnp.zeros((f_online.shape[0], ))  # (146, ), as MSE is just one number
